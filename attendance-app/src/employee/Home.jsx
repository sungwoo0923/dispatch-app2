...
export default function Home() {
  ...
  // 관리자가 근로자등록 시 "자동발송"을 체크해뒀다면, 이 직원이 모바일에
  // 처음 로그인해 홈 화면을 열자마자(관리자가 직접 발송 버튼을 누를 필요
  // 없이) 회사 직인이 이미 날인된 표준근로계약서를 자동으로 생성한다.
  // 위 pendingContracts 카운트가 즉시 반영되어 아래 온보딩 안내 팝업이
  // "근로계약서 서명하기"로 곧바로 안내한다. 이미 계약서가 하나라도
  // 있으면(재로그인 등) 다시 만들지 않는다.
  useEffect(() => {
    if (!user || !profile?.autoSendContract || loadingSite) return;
    let cancelled = false;
    (async () => {
      ...
      await addDoc(collection(db, "contracts"), {
        ...
        content: buildDefaultContract({
          employeeName: profile.name,
          hireDate: profile.hireDate || today,
        ...
        companySignedAt: stampUrl ? today : null,
        autoGenerated: true,
  ...
  }, [user, profile?.autoSendContract, profile?.companyId, loadingSite]);

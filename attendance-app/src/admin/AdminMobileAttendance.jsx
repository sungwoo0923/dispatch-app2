...
import { collection, query, where, onSnapshot, getDocs, updateDoc, addDoc, deleteDoc, doc, serverTimestamp } from "firebase/firestore";
import { ChevronLeft, ChevronRight, Search, Phone, Clock, CalendarDays, Trash2 } from "lucide-react";
...
export default function AdminMobileAttendance() {
  ...
  // PC 출근현황과 달리 이 화면엔 출근기록 삭제 기능이 없어, 탈퇴 처리 등으로
  // 더 이상 의미 없어진 기록을 관리자가 정리할 방법이 없었다 — 상세 팝업에
  // 삭제 버튼을 추가한다.
  const deleteRecord = async () => {
    if (!detail) return;
    if (!(await confirm("이 출근기록을 삭제하시겠습니까?", "delete"))) return;
    await deleteDoc(doc(db, "attendance", detail.record.id));
        ...
        footer={
          editMode ? (
            ...
            <>
              ...
              <Button className="flex-1" onClick={() => setEditMode(true)}>
                수정
              </Button>
            </>
          )

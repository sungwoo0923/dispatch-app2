        ...
        <div className="fixed inset-0 z-[9999] flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-3xl max-h-[88vh] flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-5 py-3 border-b border-gray-100 shrink-0">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-[15px] font-bold text-[#1B2B4B]">과거 운임 조회</div>
                  <div className="text-[11px] text-gray-400 mt-0.5 flex flex-wrap gap-1">
                    {form.상차지명 && <span className="bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded-md font-medium">{form.상차지명}</span>}
                    {form.하차지명 && <><span className="text-gray-300">→</span><span className="bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-md font-medium">{form.하차지명}</span></>}
                    {form.화물내용 && <span className="bg-orange-50 text-orange-500 px-1.5 py-0.5 rounded-md font-medium">{form.화물내용}</span>}
                    {form.톤수 && <span className="bg-green-50 text-green-600 px-1.5 py-0.5 rounded-md font-medium">{form.톤수}</span>}
                  </div>
                </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2.5">
              ...
              {fareMatches.map((r, i) => {
                ...
                const dateLabel = (() => {
                  if (!o.상차일) return "-";
                  const diff = Math.floor((Date.now() - new Date(o.상차일).getTime()) / 86400000);
                  if (diff === 0) return "오늘";
                  if (diff === 1) return "어제";
                ...
                return (
                  <div key={i} className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
                      ...
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 mb-1">
                            ...
                            <span className="text-[11px] text-gray-400">{dateLabel} · {o.상차일 || ""}</span>
                          </div>
                          {/* 경로 */}
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            {o.화물내용 && (
                              <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-medium ${r.tags.includes("화물일치") ? "bg-orange-100 text-orange-700" : r.tags.includes("화물유사") ? "bg-orange-50 text-orange-500" : "bg-gray-100 text-gray-500"}`}>
                                {o.화물내용}
                              </span>
                            )}
                            {o.톤수 && (
                              <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-medium ${r.tags.includes("톤수일치") ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                                {o.톤수}
                              </span>
                            )}
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <div className="text-[15px] font-extrabold text-[#1B2B4B]">{r.claim.toLocaleString()}원</div>
                    ...
                    <button
                      ...
                      className="w-full py-2.5 bg-[#1B2B4B] text-white text-[12px] font-bold text-center active:opacity-80"
                    >
                      이 운임 적용하기
                    </button>
                  </div>
 ...
 // 🔧 파렛트 추출 (1파 / 2파 / 3파렛트 / 3PLT 대응)
  ...
  const copy = async (type) => {
    ...
    ========================= */
      ...
      const _pkgM=(str)=>{const s=String(str||"").trim();if(!s)return 0;const kg=s.match(/([\d.]+)\s*kg/i);if(kg)return parseFloat(kg[1]);const ton=s.match(/([\d.]+)/);return ton?parseFloat(ton[1])*1000:0;};
      const _fkgM=(kg)=>{if(!kg)return"";if(kg>=1000){const t=kg/1000;return t.toFixed(3).replace(/\.?0+$/,"")+"톤";}return`${kg}kg`;};
      const _scM=(main,stops)=>{const all=[main,...stops.map(_ctM)].filter(Boolean);const byU={};const unk=[];for(const c of all){let hit=false;for(const u of["파레트","파렛트","박스","통"]){if(c.endsWith(u)){const n=parseFloat(c.slice(0,-u.length));if(!isNaN(n)){const k=u==="파렛트"?"파레트":u;byU[k]=(byU[k]||0)+n;hit=true;break;}}}if(!hit&&c)unk.push(c);}return[...Object.entries(byU).map(([u,n])=>`${n}${u}`),...unk].join(" / ");};
      ...
      const _mainDCargoMf=order.화물내용?`\n화물내용 : ${order.화물내용}`:"";
      const _mainDTonMf=order.차량톤수?`\n화물톤수 : ${normalizeTon(order.차량톤수)}`:"";
...
`.trim();
      ...
      const _pkgMd=(str)=>{const s=String(str||"").trim();if(!s)return 0;const kg=s.match(/([\d.]+)\s*kg/i);if(kg)return parseFloat(kg[1]);const ton=s.match(/([\d.]+)/);return ton?parseFloat(ton[1])*1000:0;};
      const _fkgMd=(kg)=>{if(!kg)return"";if(kg>=1000){const t=kg/1000;return t.toFixed(3).replace(/\.?0+$/,"")+"톤";}return`${kg}kg`;};
      const _scMd=(main,stops)=>{const all=[main,...stops.map(_ctMd)].filter(Boolean);const byU={};const unk=[];for(const c of all){let hit=false;for(const u of["파레트","파렛트","박스","통"]){if(c.endsWith(u)){const n=parseFloat(c.slice(0,-u.length));if(!isNaN(n)){const k=u==="파렛트"?"파레트":u;byU[k]=(byU[k]||0)+n;hit=true;break;}}}if(!hit&&c)unk.push(c);}return[...Object.entries(byU).map(([u,n])=>`${n}${u}`),...unk].join(" / ");};
      ...
      const _mainDCargoMd=order.화물내용?`\n화물내용 : ${order.화물내용}`:"";
      const _mainDTonMd=order.차량톤수?`\n화물톤수 : ${normalizeTon(order.차량톤수)}`:"";

...
function MobileOrderCard({ order }) {
  ...
  const 상차간단 = short3(order.상차지주소 || "");
  const 하차간단 = short3(order.하차지주소 || "");

  // 배지 (당상/당착)
  const badgeConvert = (v, type) => {
    if (!v) return "";
    if (v === "당일") return type === "pick" ? "당상" : "당착";
    if (v === "내일") return type === "pick" ? "낼상" : "낼착";
    if (v === "어제") return type === "pick" ? "어제상" : "어제착";
    return v;
  };

  const 상배지 = badgeConvert(getDayBadge(order.상차일), "pick");
  const 하배지 = badgeConvert(getDayBadge(order.하차일), "drop");

  // 작업코드(지/수/직수/수도)
  const codeColor = (code) => {
    if (code === "지") return "bg-orange-300 text-white";
    if (code === "수") return "bg-yellow-300 text-black";
    if (code === "직수") return "bg-black text-white";
    if (code === "수도") return "bg-blue-200 text-black";
    return "bg-gray-200 text-gray-600";
  };

  const 상코드 = methodCode(order.상차방법);
  const 하코드 = methodCode(order.하차방법);
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl px-4 py-5 mt-4 shadow-sm active:scale-[0.99] transition">
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        <span
          className={`px-3 py-1 rounded-full text-[12px] border ${stateColor}`}
        >
          {state}
        </span>
      </div>
      ...
      <div className="mb-5">
        ...
        <div className="flex gap-2 mt-2">
          {상배지 && (
            <span className="px-2 py-0.5 rounded-full text-[12px] bg-indigo-100 text-indigo-700 font-medium">
              {상배지}
            </span>
          )}
          {상코드 && (
            <span
              className={`px-2 py-0.5 rounded-full text-[12px] font-semibold ${codeColor(
                상코드
              )}`}
            >
              {상코드}
      ...
      <div className="mb-5">
        ...
        <div className="flex gap-2 mt-2">
          {하배지 && (
            <span className="px-2 py-0.5 rounded-full text-[12px] bg-sky-100 text-sky-700 font-medium">
              {하배지}
            </span>
          )}
          {하코드 && (
      ...
      <div className="flex justify-between items-end mt-3">
        <div className="flex flex-col">
          <span className="text-[12px] text-gray-500">청구</span>
          <span className="text-[21px] font-extrabold text-gray-900 mt-0.5">
            {fmtMoney(claim)}
          </span>
        </div>

        <div className="flex flex-col items-end">
          <span className="text-[12px] text-gray-500">기사</span>
          <span className="text-[21px] font-extrabold text-blue-600 mt-0.5">
      ...
      {bottomLabel && (
        <div className="flex mt-3">
          <span className="px-3 py-1 rounded-full bg-gray-100 border text-[12px] text-gray-700">
            {bottomLabel}
          </span>
        </div>
      )}

...
// 띄어쓰기/특수문자/대소문자 무시하고 비교
const normalizeSoft = (str) =>
  String(str || "")
    .toLowerCase()
    .replace(/\s+/g, "")        // 공백 제거
    .replace(/[^\w가-힣]/g, ""); // 특수문자 제거

const isLike = (text, keyword) =>
  normalizeSoft(text).includes(normalizeSoft(keyword));

// "1.신미 2.푸드플래닛" → 2
const getDropCountFromText = (dropText) => {
  if (!dropText) return 0;
  const matches = String(dropText).match(/\d\./g);
  return matches && matches.length ? matches.length : 1; // 숫자 없으면 1곳 하차라고 봄
};

// 화물내용에서 파렛트 수 추출 (1파렛 / 1빠 / 1p 등 전부 숫자만 뽑기)
const getPalletFromCargoText = (cargo) => {
  const num = parseInt(String(cargo || "").replace(/[^\d]/g, ""), 10);
  return isNaN(num) ? null : num;
};

// ✅ 송원 → 신미/푸드플래닛 단가표
const palletFareRules = {
  double: [ // 2곳 하차 (신미 + 푸드플래닛)
  ...
  sinmi: [ // 신미 1곳 하차
    ...
    { min: 3, max: 3, fare: [180000, 200000] }, // ⬅ 3파렛: 선택
        ...
        {[
          ...
          return (
            <button
              ...
              className={`px-3 py-2 rounded border text-sm ${isBlocked
  ...
  ...emptyForm,
    ...
    const handleFareSearch = () => {
      ...
      if (palletCount != null && isLike(pickup, "송원")) {
        ...
        const hasFood = isLike(drop, "푸드플래닛");            // 오타/띄어쓰기 허용
        const hasSinmi = isLike(drop, "신미");
                ...
                {filtered.map((r, idx) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`cursor-pointer ${selected?._id === r._id ? "bg-blue-100" : ""
                        } ${idx % 2 ? "bg-gray-50" : ""}`}
                    >
                      ...
                      {/* 수수료 */}
                      <td
                        className={`${cell} text-right pr-2 ${fee < 0 ? "text-red-500" : ""
                          }`}
                      >
                        {fee.toLocaleString()}
                      </td>
...
`.trim();
  ...
  function KpiDeltaCard({ title, diff, rate }) {
    ...
    return (
      <div
        className={`rounded-2xl p-3 border shadow-sm ${up
            ? "bg-blue-50 border-blue-200"
      ...
      >
        <p className="text-xs text-gray-500">{title}</p>
        <p
          className={`text-xl font-bold mt-1 ${up ? "text-blue-700" : "text-rose-700"
            }`}
        >
          {`${diff >= 0 ? "+" : ""}${Number(diff || 0).toLocaleString()}원`}
        </p>
        <p
          className={`text-xs ${up ? "text-blue-700" : "text-rose-700"
            }`}
        >
    ...
    return (
      ...
      >
        ...
        {series.map((s, idx) => (
          <g
            key={s.key}
            transform={`translate(${padding.left + idx * 140
              }, ${padding.top + 8})`}
          >
            <rect
              width="12"
